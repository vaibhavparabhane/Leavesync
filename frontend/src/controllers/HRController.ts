...
export class HRController {
  ...
  static async fetchTeamLeaves(
    ...
    try {
      const data = await hrAPI.getTeamLeaves(page, perPage, status, sortBy, sortOrder, search);
      return { success: true, data };
    } catch (error: any) {
      return { success: false, error: ApiErrorHandler.getErrorMessage(error) };
    }
  ...
  static async applyLeaveForEmployee(leaveData: {
    user_id: string;
    ...
    try {
      const data = await hrAPI.applyLeaveForEmployee(leaveData);
  ...
  // Alias for assignHolidayToEmployees
  static async assignHolidays(employeeIds: string[], holidayId: string): Promise<ControllerResponse> {
    return this.assignHolidayToEmployees({ employee_ids: employeeIds, holiday_id: holidayId });
  }
}
